...
<body class="font-poppins bg-[#fffff] min-h-screen flex items-center justify-center px-4">
  ...
  <img src="<?php echo e(asset('images/sarpras-logo.png')); ?>" alt="Logo" class="mx-auto w-40 mb-4">
    ...
    <button onclick="document.getElementById('roleModal').classList.remove('hidden')" 
      ...
      <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M10 1a9 9 0 100 18 9 9 0 000-18zM9 14H7v-4h2v4zm4 0h-2V6h2v8z"/></svg>
      Login with Email
    </button>
  </div>
  ...
  <div id="roleModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center px-4">
    <div class="bg-white p-6 rounded-lg w-full max-w-xs space-y-3 text-center">
      <h2 class="text-base font-semibold">Login sebagai:</h2>
        <a href="<?php echo e(url('/quick-login/admin')); ?>" class="block w-full py-2 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm">Admin</a>
        <a href="<?php echo e(url('/quick-login/mahasiswa')); ?>" class="block w-full py-2 bg-green-500 text-white rounded hover:bg-green-600 text-sm">Mahasiswa</a>
        <a href="<?php echo e(url('/quick-login/bem')); ?>" class="block w-full py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-sm">BEM</a>
        <a href="<?php echo e(url('/quick-login/dosen')); ?>" class="block w-full py-2 bg-red-500 text-white rounded hover:bg-red-600 text-sm">Dosen</a>
